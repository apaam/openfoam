...
# Runtime image tag uses the product version (phynexis-foam:2412-arm64), not
# the Ubuntu base version.
DOCKER_OPENFOAM_IMAGE = \
  $(DOCKER_OPENFOAM_IMAGE_NAME):$(OPENFOAM_DIST_VERSION)-$(DOCKER_IMAGE_SUFFIX)
OPENFOAM_DIST_VERSION := $(patsubst v%,%,$(OPENFOAM_VERSION))
DOCKER_DIST_BASENAME := phynexis-foam-docker-$(OPENFOAM_DIST_VERSION)-linux-$(DOCKER_IMAGE_SUFFIX)
