...
if [[ -d "${STAGE}/lib/bundled" ]] && [[ ! -f "${PREFS_SYS_OPENMPI}" ]]; then
  mkdir -p "$(dirname "${PREFS_SYS_OPENMPI}")"
  cat >"${PREFS_SYS_OPENMPI}" <<EOF
# Generated by rewrite_openfoam_prefs.sh for dist-native / Docker.
export MPI_ARCH_PATH="${BUNDLED}"
EOF
