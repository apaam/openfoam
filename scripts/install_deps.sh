...
case "$(uname -s)" in
Darwin)
  echo "[install-deps] brew bundle (${ROOT}/Brewfile)"
  brew bundle -f --file="${ROOT}/Brewfile"
  ;;
Linux)
  if ! command -v apt-get >/dev/null 2>&1; then
    echo "[install-deps] Unsupported Linux: need apt-get (Ubuntu/Debian)" >&2
    exit 1
  fi
  pkgs_file="${ROOT}/scripts/linux_build_packages.txt"
  mapfile -t pkgs < <(grep -v '^#' "${pkgs_file}" | grep -v '^[[:space:]]*$')
  echo "[install-deps] apt install (${#pkgs[@]} packages from linux_build_packages.txt)"
  run_apt apt-get update
  run_apt apt-get install -y "${pkgs[@]}"
  echo "[install-deps] pip: setuptools wheel mpi4py"
  python3 -m pip install --break-system-packages setuptools wheel mpi4py
  ;;
*)
  echo "[install-deps] Unsupported OS: $(uname -s)" >&2
  exit 1
  ;;
esac
